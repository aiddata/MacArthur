...
#Load in data
ftp_url <- "ftp://reu:reudata@ftp.aiddata.wm.edu/REU/MacArthur/"
...
{
  cur_file <- paste(mDir, "/analysis_data/", files[[i]], sep="")
  #Download the newest revision of the file if we don't have it already.
  if(!file.exists(cur_file))
  {

    dl_url <- paste(ftp_url,"/",files[[i]], sep="")
    print(dl_url)
    print(i)
    download.file(dl_url, destfile=cur_file, method="libcurl")
...
csv <- paste(mDir, "/analysis_data/sea.csv", sep="")
json <- paste(mDir, "/analysis_data/sea.json", sep="")
dta <- read.csv(csv)

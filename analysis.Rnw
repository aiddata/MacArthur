...
\end{equation}
where \textit{h} represents each spatial bin, \textit{N} the number of spatial units, \textit{i} and \textit{j} are indexes for each unit, \textit{X} is the variable of itnerest, and \begin{math}W_{ij}\end{math} represents the weights matrix.  In this application, the weights matrix is specified according to the bin (\textit{h}) being analyzed.  A best-fit gaussian spatial decay function is then found to approximate the distance decay observed on the Morans I measurements. Based on this distance-decay function, the estimated impact of chinese projects is weighted based on the cumulative distance-weighted average across all chinese projects.
\\

<<Calc_correl, echo=FALSE, include=FALSE, cache=TRUE>>=
correlogram_data <- correlog(x = coordinates(AOI_cells)[,1], y = coordinates(AOI_cells)[,2], z=AOI_cells$lnyx_1999e, increment=50, latlon=TRUE, na.rm=TRUE, resamp=10)
@
  \begin{figure}[H]
...
As this correlogram illustrates, a lack of positive spatial autocorrelation among LTDR measurements in 1999 is first observed at approximately \Sexpr{round(as.numeric(correlogram_data$x.intercept))} km.  We use this distance as a threshold to screen nearby Chinese aid projects - i.e., the average and sum distance of all Chinese aid projects within \Sexpr{round(as.numeric(correlogram_data$x.intercept))} km is taken for each cell, and projects beyond that distance are not included.  These two values - the average and summed distance of Chinese aid projects - are used as our approximation of the strength of Chinese interventions within any given cell on the landscape.
\par
...
AOI_cells$tot_proj <- apply(thresh_dMatrix, 2, function(x) length(which(!is.na(x))))
AOI_cells$totDist <- total_distance_km
AOI_cells$avgDist <- AOI_cells$totDist / AOI_cells$tot_proj 
@
...
\label{Proj_Avg_Dist}
\centering
<<Treatment_Map, echo=FALSE, fig="TRUE", out.width="0.8\\linewidth">>=
mapAvg = spplot(AOI_cells, "avgDist", main=paste("Average Distance to Chinese Aid Projects (within ",round(correlogram_data$x.intercept)," km)", sep=""), sub="Kilometers", sp.layout=list(Mac_spdf, pch=16, col="red", cex=1.0))
mapCnt = spplot(AOI_cells, "tot_proj", main=paste("Total Nearby (within ",round(correlogram_data$x.intercept)," km) Chinese projects", sep=""), sub="Kilometers", sp.layout=list(Mac_spdf, pch=16, col="red", cex=1.0))
par(mfrow=c(2,1))
mapAvg
mapCnt
@

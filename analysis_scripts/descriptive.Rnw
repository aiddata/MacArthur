...
<<DataLoad, include=FALSE, cache=TRUE>>=
#Load in data
...
\begin{figure}[H]
\caption{Histogram of LTDR NDVI Values in (a) locations with >50\% forest cover in Hansen 2000, and (b) locations with <\Sexpr{forest_thresh}\% forest cover in Hansen 2000.}
